            ...
            GamingZoneTheme {
                ...
                val shouldShowBottomNav = remember(currentDestination) {
                    val shouldShow = BottomNavItem.entries.any { item ->
                        val currentRoutes =
                            currentDestination?.hierarchy?.mapNotNull { it.route }.toString()
                        ...
                        val matches = currentDestination?.hierarchy?.any { entry ->
                            ...
                            Log.d(
                                "Navigation",
                                "Checking ${item.title}: Current Entry Route: $entryRoute, Bottom Item Route: $bottomItemRoute, EndsWith: $endsWith"
                            )
                            endsWith
                        } == true
                        matches
                    ...
                    Log.d(
                        "Navigation",
                        "Final shouldShowBottomNav for ${currentDestination?.route}: $shouldShow"
                    )
                    shouldShow
                }
                    ...
                    if (shouldShowBottomNav) {
                        ...
                        snapshotFlow { scrollBehavior.state.contentOffset }
                            ...
                            .onEach { currentOffset ->
                                ...
                                Log.d(
                                    "MainActivity",
                                    "Offset Listener - Current: $currentOffset, Previous: $previousOffset, Delta: $delta"
                                )
                                ...
                                if (delta < scrollThreshold) {
                                    ...
                                    Log.d(
                                        "MainActivity",
                                        "Offset Listener - Scrolling DOWN, hiding bottom bar"
                                    )
                                    showBottomBar = false
                            ...
                            .collectLatest {
                                Log.d(
                                    "MainActivity",
                                    "Debounce - Scrolling STOPPED, showing bottom bar"
                                )
                                showBottomBar = true
                            }
                    } else {
                ...
                Scaffold(
                    ...
                    bottomBar = {
                        Log.d(
                            "MainActivity",
                            "shouldShowBottomNav (in bottomBar): $shouldShowBottomNav"
                        )
